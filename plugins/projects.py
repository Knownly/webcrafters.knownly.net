...
def preBuild(site):
    ...
    for page in site.pages():
        if page.path.startswith(POSTS_PATH) and not page.path.endswith('index.html'):
            ...
            postContext['start_date'] = find('start_date')
            postContext['short_description'] = find('short_description')
            postContext['path'] = '/%s' % page.path
            postContext['body'] = getNode(get_template(page.path), name="body")
            
            # Parse the date into a date object
            try:
                postContext['start_date'] = datetime.datetime.strptime(postContext['start_date'], '%d-%m-%Y')
            except Exception, e:
    ...
    POSTS = sorted(POSTS, key=lambda x: x['start_date'])
    POSTS.reverse()
