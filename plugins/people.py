...
def preBuild(site):
    ...
    for page in site.pages():
        if page.path.startswith(POSTS_PATH) and not page.path.endswith('index.html'):
            ...
            postContext['first_attended'] = find('first_attended')
            postContext['path'] = '/%s' % page.path
            postContext['body'] = getNode(get_template(page.path), name="body")
            
            # Parse the date into a date object
            try:
                postContext['first_attended'] = datetime.datetime.strptime(postContext['first_attended'], '%d-%m-%Y')
            except Exception, e:
    ...
    POSTS = sorted(POSTS, key=lambda x: x['first_attended'])
    POSTS.reverse()
